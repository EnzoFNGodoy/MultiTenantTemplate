...
public sealed class CategoryController : ControllerBase
{
    ...
    [HttpPost]
    public async Task<IActionResult> Post(RequestCategoryViewModel viewModel)
        => Ok(await _categoryServices.Create(viewModel));

    [HttpPut("{id}")]
    public async Task<IActionResult> Update(Guid id, RequestCategoryViewModel viewModel)
